            ...
            with st.expander(f"🎨 {row['customer_name']} - {row['title']} (Abgabe: {row['delivery_date']} | Status: {row['status']})"):
                ...
                with col_a:
                    ...
                    if new_price != current_price:
                        import sqlite3
                        conn = sqlite3.connect("kuenstler_auftraege.db")
                        c = conn.cursor()
                        c.execute("UPDATE orders SET price = ? WHERE id = ?", (new_price, row['id']))
                        conn.commit()
                        conn.close()
                        st.success("Preis erfolgreich aktualisiert!")
                        st.rerun()

                    # 2. TERMIN (ABGABEDATUM) ÄNDERN
                    import datetime
                    try:
                    ...
                    if str(new_due_date) != str(row.get('delivery_date')):
                        ...
                        conn = sqlite3.connect("kuenstler_auftraege.db")
                        c = conn.cursor()
                        c.execute("UPDATE orders SET delivery_date = ? WHERE id = ?", (str(new_due_date), row['id']))
                        conn.commit()
                        conn.close()
                    ...
                    current_status = row.get('status') if row.get('status') in status_options else "Neu"
                ...
                # --------------------------------------------------------
                # RECHTE SPALTE: Bild-Anzeige und Upload
                # --------------------------------------------------------
                with col_b:
                    ...
                    if row.get('image_path') and isinstance(row['image_path'], str) and os.path.exists(row['image_path']):
                        ...
                        if new_img is not None:
                            ...
                            conn = sqlite3.connect("kuenstler_auftraege.db")
                            c = conn.cursor()
                            c.execute("UPDATE orders SET image_path = ? WHERE id = ?", (img_path, row['id']))
                            conn.commit()
                            conn.close()
